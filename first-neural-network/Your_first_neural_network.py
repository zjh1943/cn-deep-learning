...
class NeuralNetwork(object):
    ...
    def train(self, features, targets):
        ...
        for X, y in zip(features, targets):
            ...
            output_error_term = error # 线性函数
            
            # TODO: Calculate the hidden layer's contribution to the error
            hidden_error = np.dot(output_error_term,self.weights_hidden_to_output.T)
            hidden_error_term = hidden_error * sigmoid_output_2_derivative(hidden_outputs)
                                                                                                                                          
            # TODO: Backpropagated error terms - Replace these values with your calculations.
            # output_error_term = error_delta
            # hidden_error_term = np.dot(X.T, sigmoid_output_2_derivative(hidden_outputs))

            # Weight step (hidden to output)
            delta_weights_h_o += -np.dot(hidden_outputs.T, output_error_term)*self.lr
